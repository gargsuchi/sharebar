...
use Drupal\drupalmoduleupgrader\Converter\Routing\RouterBuiltEvent;
use Drupal\drupalmoduleupgrader\Converter\Routing\RouteWrapperInterface;
...
class RouteWrapper extends ArrayCollection implements RouteWrapperInterface {
  ...
  /**
   * @var \Drupal\drupalmoduleupgrader\Converter\Routing\Drupal7\Router
   */
  protected $children;
  ...
  public function onRouterBuilt(RouterBuiltEvent $event) {
    ...
    $this->siblings = $this->router
      ->filter(function(RouteWrapper $route) use ($parent, $my_path, $my_length) {
        $path = $route->getPath();
        // <WTF><sarcasm>strPos(), in its wisdom, won't cast to string.</sarcasm></WTF>
   ...
   *
   * @return \Drupal\drupalmoduleupgrader\Converter\Routing\Drupal7\Router
   */
